...
function hasAlbumPassword(value) {
...
function normalizeDimensionsForOrientation(metadata) {
  ...
  // EXIF orientation 5-8 means the rendered image is rotated 90/270 degrees.
...
async function processAlbum({ sharp, albumsRoot, publicRoot, folderName }) {
  ...
  const requiresPassword = isAlbumProtected(albumMeta);
  ...
  for (const file of files) {
    ...
    const normalized = normalizeDimensionsForOrientation(metadata);
    items.push({
      id: stem,
      filename: file,
...
async function buildAlbumsAccessIndex(albumsRoot) {
  const index = {
    version: 1,
  ...
  for (const folderName of folders) {
    const albumJsonPath = path.join(albumsRoot, folderName, "album.json");
    if (!(await exists(albumJsonPath))) {
      continue;
    }
    ...
    try {
      ...
      const slug = String(albumMeta.slug || folderName);
      index.albums[slug] = {
        requiresPassword: isAlbumProtected(albumMeta),
      };
    } catch (error) {
      console.warn(`- 跳过 access-index 条目 ${folderName}:`, error);
    }
  }
...
async function main() {
  ...
  if (results.length === 0) {
    ...
    // Even if image assets were not produced, refresh access index for middleware.
  }

  const accessIndex = await buildAlbumsAccessIndex(albumsRoot);
  await fs.writeFile(
    path.join(publicRoot, "access-index.json"),
    `${JSON.stringify(accessIndex, null, 2)}\n`,
    "utf8",
  );

  if (results.length === 0) {
    console.log("没有生成图片资源，已刷新 access-index。");
    return;
  }
  ...
  console.log("- access-index: public/albums/access-index.json");
}
