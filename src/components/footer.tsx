...
export function Footer() {
  ...
  const nextVersion = normalizeVersion(nextPackageJson.version);
  const appVersion = normalizeVersion(packageJson.version);
    ...
    <footer className="border-t border-border/60">
      <div className="mx-auto flex w-full max-w-4xl flex-col items-center gap-1 px-4 py-4 text-xs text-muted-foreground md:flex-row md:justify-between md:gap-2 md:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span aria-hidden="true">·</span>
          <span>{`Next.js ${nextVersion} / v${appVersion}`}</span>
        </div>
      </div>
    </footer>
  );
